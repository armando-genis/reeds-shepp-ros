...
std::tuple<std::vector<double>, std::vector<double>, std::vector<double>, std::vector<int>> generate_local_course(
    ...
    for (size_t i = 0; i < mode.size(); ++i) {
        ...
        while (std::abs(pd) <= std::abs(lengths[i])) {
            ind += 1;
            std::tie(px, py, pyaw, directions) = interpolate(ind, pd, mode[i], maxc, ox, oy, oyaw, px, py, pyaw, directions);
            pd += d;
        }

        ll = lengths[i] - pd - d;
        ind += 1;
        std::tie(px, py, pyaw, directions) = interpolate(ind, lengths[i], mode[i], maxc, ox, oy, oyaw, px, py, pyaw, directions);
    }

    px.erase(std::remove(px.begin(), px.end(), 0.0), px.end());
    py.erase(std::remove(py.begin(), py.end(), 0.0), py.end());
    pyaw.erase(std::remove(pyaw.begin(), pyaw.end(), 0.0), pyaw.end());
